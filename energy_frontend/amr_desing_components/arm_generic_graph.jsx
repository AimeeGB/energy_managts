...
const GenericGraph = ({ contador }) => {
    ...
    const API_URL = 'http://127.0.0.1:8000/amr_api/data-views/';
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            try {
                const urlParams = new URLSearchParams({
                    contador_numero: contador.numeroContador,
                    intervalo,
        ...
        fetchData();
    }, [contador, intervalo, ultimas24Horas, fechaEspecifica]);
    ...
    const options = {
        ...
        plugins: {
            ...
            title: {
                display: true,
                text: `Consumo Energético - ${contador?.nombreContador || ''} - ${fechaGrafico || 'Últimos datos'}`,
            }
        }
    };
    ...
    return (
        <Card className="shadow-sm">
            <Card.Header className="bg-dark text-white">
                {/* <h5 className="mb-0">{contador?.nombreContador || 'Contador'} - Monitoreo Energético</h5> */}
                <h5 className="text-center mb-0">{contador?.nombreContador || 'Contador'}</h5>
            </Card.Header>
            <Card.Body>
